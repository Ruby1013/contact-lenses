...
const catalog=read('public/bblens-catalog.json'),products=read('public/products.json'),before=read('audit/bblens-2026-09-22/before.json');
const isBB=p=>p.url.includes('bblens.tw');
assert.deepEqual(products.filter(p=>!isBB(p)),before.filter(p=>!isBB(p)));
assert.equal(catalog.length,234);assert.equal(new Set(catalog.map(p=>p.url)).size,234);
...
for(const p of offers){const q=catalog[+p.sourceProductId];assert.equal(p.url,q.url);assert.equal(p.totalPieces,p.piecesPerBox*(p.boughtBoxes+p.giftBoxes));assert.equal(p.unitPrice,p.salePrice/p.totalPieces);assert.match(p.checkedAt,/^2026-09-22/);if(!p.offerType)assert.equal(p.salePrice,q.price);}
...
console.log('PASS: 234 catalog entries, 157 offers, current promotions, package quantities, brand isolation, other retailers unchanged, search and sorting.');
