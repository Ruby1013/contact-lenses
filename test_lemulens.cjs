...
const catalog=read('public/lemulens-catalog.json'),data=read('public/products.json'),before=read('audit/lemulens-2026-09-23/before.json');
...
for(const p of offers){
 const c=catalog.find(c=>c.id===p.sourceProductId);assert.ok(c);assert.equal(c.url,p.url);assert.ok(!c.name.includes('暢飲大包組'));assert.match(p.checkedAt,/^2026-09-23/);
...
assert.ok(fs.readFileSync(path.join(__dirname,'public/lemulens-prices-2026-09-23.xlsx')).subarray(0,2).equals(Buffer.from('PK')));
