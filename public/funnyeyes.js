...
fetch('funnyeyes-catalog.json?v=funnyeyes-20260923').then(r=>{if(!r.ok)throw Error('HTTP '+r.status);return r.json();}).then(data=>{data=data.map(p=>({...p,categories:p.categories.map(c=>c.name)}));catalog=data;for(const name of [...new Set(data.flatMap(p=>p.categories))].sort()){const option=document.createElement('option');option.value=name;option.textContent=name;el('category').append(option);}renderCatalog();for(const id of ['query','category','sort'])el(id).addEventListener('input',renderCatalog);}).catch(()=>{el('count').textContent='商品資料暫時無法載入，請重新整理或下載 Excel 價格表。';});
