...
function selectCatalog(data,query,sort,availability='all'){
  ...
  const rows=data.filter(p=>(!text||`${p.brand} ${p.name} ${p.condition} ${p.promo}`.toLocaleLowerCase().includes(text))&&(availability==='all'||p.soldOut===(availability==='soldout')));
...
function renderCatalog(){
  ...
  for(const p of rows){
    ...
    for(const [i,value] of [p.name,p.price.toLocaleString('zh-TW'),p.listPrice?.toLocaleString('zh-TW')??'—',p.condition,p.promo||'—',p.soldOut?'暫無庫存':'未標示缺貨'].entries()){
      const td=document.createElement('td');td.textContent=value;if(i===1||i===2)td.className='money';if(i===5&&p.soldOut)td.className='sold-out';tr.append(td);
    }
...
fetch('watsons-catalog.json').then(r=>{if(!r.ok)throw Error(`HTTP ${r.status}`);return r.json();}).then(data=>{
