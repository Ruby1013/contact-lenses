...
for(const p of offers){const q=catalog.find(q=>q.id===p.sourceProductId);assert.ok(q);assert.equal(p.salePrice,q.detail_price);assert.equal(p.product,q.name);assert.match(p.checkedAt,/^2026-09-22/);assert.equal(p.unitPrice,p.salePrice/(p.productType==='solution'?p.totalVolumeMl:p.totalPieces));}
