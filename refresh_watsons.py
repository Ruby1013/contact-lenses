...
def run():
    raw = json.loads((ROOT.parent / 'outputs/01a0c9b6-watsons/products.json').read_text(encoding='utf-8'))
    catalog = []
    for r in raw:
        ...
        catalog.append(dict(id=r['url'].split('/')[-1], brand=r['brand'], name=r['name'],
            ...
            url=r['url'], checkedAt='2026-09-22', rawPrice=r['price']))
    ...
    shutil.copyfile(ROOT.parent/'outputs/01a0c9b6-watsons/屈臣氏隱形眼鏡價格表_2026-09-22.xlsx',PUBLIC/'watsons-prices-2026-09-22.xlsx')
    ...
    audit=ROOT/'audit/watsons-2026-09-22';audit.mkdir(parents=True,exist_ok=True)
    ...
    line='      <p>屈臣氏價格核對：<time datetime="2026-09-22">2026/09/22</time>。<a href="watsons.html">查詢全部 206 項商品價格</a> · <a href="watsons-prices-2026-09-22.xlsx" download>下載屈臣氏 Excel 價格表</a></p>'
