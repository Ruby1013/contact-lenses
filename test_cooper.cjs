...
assert.deepEqual(Array.from(modes, m => m.products.length), [4, 3, 1]);
...
assert.equal(split[0].products[0].source, 'A'); // 9.17 beats 9.67, despite lower checkout at B
...
console.log('PASS: Cooper mixed/single/bulk rankings, same-shop offers, gifts, precise costs and empty groups.');
