...
酷柏每個品項分為兩種排行榜：單盒比價、量販比價。買 1 盒（含附贈）歸單盒，需買 2 盒以上歸量販；先分購買門檻，再在各榜選每個商家最便宜的方案。品項沿用整體至少三站的收錄門檻，子榜不足三家照實顯示，沒有資料則標示尚未收錄。酷柏依總價／實得總片數的完整精度排名，單價顯示兩位小數。執行 `node test_cooper.cjs` 驗證。
